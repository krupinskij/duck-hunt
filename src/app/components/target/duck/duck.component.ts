...
@Component({
  ...
  animations: [
    trigger('fly', [
      ...
      transition(
        `${DuckState.FlyHorizontally} <=> ${DuckState.FlyVertically}`,
        animate('{{ time }}s'),
        { params: { time: '1s' } }
      ),
      transition(
        `${DuckState.FlyHorizontally} => ${DuckState.Shot}`,
        animate('{{ time }}s'),
        { params: { time: '1s' } }
      ),
      transition(
        `${DuckState.FlyVertically} => ${DuckState.Shot}`,
        animate('{{ time }}s'),
        { params: { time: '1s' } }
      ),
      ...
      transition(
        `${DuckState.FlyHorizontally} => ${DuckState.Flee}`,
        animate('5s')
      ),
      transition(
        `${DuckState.FlyVertically} => ${DuckState.Flee}`,
        animate('5s')
      ),
    ]),
  ],
})
export default class DuckComponent
  ...
  implements OnInit, OnDestroy
{
  ...
  setDuckState(state: DuckState) {
    switch (state) {
      case DuckState.FlyVertically:
        ...
        [this.nextPoint, this.prevPoint] = this.calculateBasePosition(
          this.nextPoint
        );
      ...
      case DuckState.FlyHorizontally:
        this.duckState = state;
        [this.nextPoint, this.prevPoint] = this.calculateWallPosition(
          this.nextPoint
        );
        this.time = this.calculateTime(this.config.speed);
        this.image = this.setImage(this.prevPoint, this.nextPoint, state);
  ...
  private _messangerHandler({ payload: { action } }: Message) {
    if (
      ![DuckState.FlyHorizontally, DuckState.FlyVertically].includes(
        this.duckState
      )
    )
      return;
